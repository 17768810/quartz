...
func Parse(spec string) (Schedule,error) {
	...
	if len(fields) != 5 && len(fields) != 6 {
		log.Println("Expected 5 or 6 fields, found %d: %s", len(fields), spec)
...
func parseDescriptor(spec string) (Schedule,error) {
	...
	if strings.HasPrefix(spec, every) {
		...
		if err != nil {
			log.Println("Failed to parse duration %s: %s", spec, err)
			return nil,err
		}
		return Every(duration),nil
	}

	log.Println("Unrecognized descriptor: %s", spec)
